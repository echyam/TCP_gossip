...
void hello_received(struct file_info *fi, char *addr_port){
    ...
    *--p = ':';
    ...
    gossip_str_bldr(sockaddr_to_file(my_addr));
}

// helper to build the line for a gossip
void gossip_str_bldr(struct file_info *fi){
    char msg[512]; // char array representing gossip message string
    char ct_str[25];
    struct file_info *fi2 = file_info;

    counter++;
    strcpy(msg, addr_to_string(my_addr));
    strcat(msg, "/");

    sprintf(ct_str, "%d", counter);
    strcat(msg, ct_str);
    strcat(msg,"/");

    // payloads
    while(fi2 != NULL){
        if ((fi2->type == FI_OUTGOING && fi2->u.fi_outgoing.status == FI_CONNECTED)|| (fi2->type == FI_INCOMING)) {
            if(fi2->status == FI_KNOWN){
                strcat(msg,";");
                strcat(msg, addr_to_string(fi2->addr));
            }
        }
        fi2 = fi2->next;
    }
    //strcat(msg, "\n");
    gossip_received(fi, msg, nl);
    //printf("got through gossip_str_bldr\n");
}
...
static void handle_line(struct file_info *fi, char *line){
    ...
    case 'C':
        ...
        break;
    case 'G':
        gossip_received(fi, &line[1], nl);
        break;
    case 'H':
        hello_received(fi, &line[1]);
        break;
    case 'S':
        send_received(fi, &line[1], nl);
        break;
    case 'E':
    case 'e':
        exit(0);
...
void try_connect(struct file_info *fi){
     ...
     */
    // YOUR CODE HERE
    fcntl (skt, F_SETFL, O_NONBLOCK|fcntl(skt,F_GETFL,0));

    /* end TCP create nonblocking socket */
...
int main(int argc, char *argv[]){
    ...
    int slen = 1;
    setsockopt(skt, SOL_SOCKET, SO_REUSEADDR, &slen, sizeof(slen));
     ...
     */
    // YOUR CODE HERE
    bind (skt, (struct sockaddr*) &bind_addr, sizeof(bind_addr));
